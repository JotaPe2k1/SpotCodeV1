...
const Music = (props) => {
  let PlayingButton
  if (props.playing) {
    PlayingButton = <FaStopCircle size='45px' className='has-text-dark' onClick={() => props.setPlaying([])} />
  } else {
    PlayingButton = <FaPlayCircle size='45px' className='has-text-dark' onClick={() => props.setPlaying(props.song)} />
  }
  return (
    <Fragment>
